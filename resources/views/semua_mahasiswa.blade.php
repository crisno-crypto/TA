            ...
            <div class="container-fluid">
                <div class="row bg-title">
                    <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                        <h4 class="page-title">input nilai mahasiswa</h4> </div>
                    
                </div>
                <!-- /.row -->
                <!-- .row -->

            <form action="/inputnilai/tambahnilai" method="post">
            {{ csrf_field() }}
                @foreach($kelas as $data)
                <h4>Prodi :  {{ $data->prodi }}</h2>
                <h4>Konsentrasi : {{ $data->konsentrasi }}</h2>
                <h4>Kelas : {{ $data->kelas }}</h2>
                <input type="hidden" placeholder="" name="id_kelas" class="form-control form-control-line" value="{{ $data->id_kelas }}">
              @endforeach
              @foreach($mks as $datam)
                <h4>Semester :  {{ $datam->semester }}</h2>
                <input type="hidden" placeholder="" name="semester" class="form-control form-control-line" value="{{ $datam->semester }}">
                <h4>Matakuliah :  {{ $datam->kd_mk }}</h2>
                <input type="hidden" placeholder="" name="id_mk" class="form-control form-control-line" value="{{ $datam->id_mk }}">
              @endforeach
              <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach($mahasiswa as $datamah)
               
                <tr>
                <td> {{ $datamah->nim }}</td>
                <input type="hidden" placeholder="" name="id_mhs[]" class="form-control form-control-line" value="{{ $datamah->id_mhs }}">
                <td> {{ $datamah->nama }}</td>
                <td>
                    <select class="form-control form-control-line" name="nilai[]">
